...
function FunctionCallOutput({
  ...
  const colorBoxes = colors.map((color: any) => (
    <div
      key={color}
      className="flex h-16 w-full items-center justify-center rounded-md border border-gray-200"
      style={{ backgroundColor: color }}
    >
      <p className="rounded-md border border-black bg-slate-100 p-2 text-sm font-bold text-black">
        {color}
      </p>
    </div>
  ));
        ...
        if (
          ...
          setTimeout(() => {
            sendClientEvent({
              ...
              response: {
                instructions: `
                  you speak english interogate the user with only 2 questions, his name and age then say good bye to him 
                `,
              },
            });
          }, 500);
